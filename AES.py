...
def SBOX(val):
    ...
    return '{:02x}'.format(S_BOX[index])
     
def invSBOX(val):
    index=int(val[0],16)*16 + int(val[1],16)
    return '{:02x}'.format(inv_S_BOX[index])
...
def shiftRows(matrix):
  ...
  shamt=0
  for row in range(0,4):
      out[row]=matrix[row][shamt:]+matrix[row][:shamt]
      shamt+=1
  return out
...
def invshiftRows(matrix):
  ...
  shamt=0
  for row in range(0,4):
      out[row]=matrix[row][4-shamt:]+matrix[row][:4-shamt]
      shamt+=1
  return out
...
def checkoverflow(num):
    if(num> 0xff):
      num &= 0xff  #make temp 8 bits only
      num = num ^ 0x1b  #xor with field polyn
    return num
def shift(num,shamt):
           
   for i in range(0,shamt):
      num=num<<1
      num=checkoverflow(num)
   return num
