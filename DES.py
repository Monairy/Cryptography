...
def permutation1(binary):       
    result=""
    for i in PC1:
            result+=binary[i-1]
    return result

def leftShift(binary, shamt):
    left=binary[shamt:28]+binary[0:shamt]
    right=binary[28+shamt:56]+binary[28:28+shamt]
    return left + right

def permutation2(binary):
    result=""
    for i in PC2:
            result+=binary[i-1]
    return result    
...
def initial_p(plainbinary):
    result=""
    for i in IP:
            result+=plainbinary[i-1]
    return result
...
def genSK(key):
     sk=list()
     perm1=permutation1(key)
     aftershift=perm1
     for i in range(0,16):
           aftershift=leftShift(aftershift,shamt[i])
